...
export default class API {
  ...
  static async Random() {
    ...
    const now = +new Date;
    console.log('[service:drinks] Random drinks');
    ...
    console.log(`[service:drinks] Random finished. took:${+new Date - now}ms`);
  ...
  static async Retrieve( id ) {
    ...
    const now = +new Date;
    console.log(`[service:drinks] Retrieve drink ${id}`);
    let url = `${BASE}/lookup.php?i=${id}`;

    let res = await fetch(url);
    console.log(`[service:drinks] Retrieve drink ${id} finished. took:${+new Date - now}ms`);
